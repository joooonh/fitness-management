...
#### 4) 느낀점
  ...
  - 처음 코드를 구현할 때는 System.out.println() 메소드를 통해 일일이 로그를 작성하였다. 그런데 구체적인 로그 정보를 알기 어려웠고 시스템 로그를 사용하지 않는 것이 좋다는 피드백을 받았다. 피드백을 수용하여 Slf4j 등의 로깅 프레임워크를 공부하여 시스템 로그와의 차이점을 익히고, @slf4j 로그를 사용하여 다시 코드를 구현했다.
